...
- Add a restrained green-toned interface with five explicit regions derived from the supplied Battle Pass screens: a slim header with route-profile, locale, and compact setup controls; a left single-page reward rail; a center current-day route workspace; a right selected-stop context rail; and a contiguous footer document ribbon plus the asset disclaimer. The regions share one continuous stage instead of nesting bordered cards inside panels.
