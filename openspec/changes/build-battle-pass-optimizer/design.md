...
The desktop information architecture has five explicit regions:
...
├───────────────┴────────────────────────┴─────────────────────┤
│ Footer: owned-document tray · asset disclaimer              │
└──────────────────────────────────────────────────────────────┘
```

While Battle Pass rewards remain, the left column uses an exclusive page accordion with exactly one Battle Pass page open at a time. Each page heading selects that page, and the selected page is persisted. When the selected page has no unclaimed rewards left, the accordion opens the first page that still contains an unclaimed reward. After every Battle Pass reward is claimed, the accordion is removed from the rendered rail and replaced by one Black Division Gear Crate reward with a localized requirement for ten documents of any non-Classified type. Global Claim all and Clear all actions sit in the rail heading; page bodies contain no buttons or selectors, only each reward's claimed checkbox with its Battle Pass item name and compact document requirements. Reward rows do not show artwork, long descriptions, stats, target selection, or unrelated metadata. Requirement abbreviations must remain unambiguous and expose their full localized text accessibly.

A claimed reward proves that its required documents were obtained even if the player did not enter them first. Its requirements therefore contribute to the header's document progress independently of the manually entered owned inventory. Reward claims do not raise, constrain, or rewrite any document quantity control; every counter continues to represent only the player's entered inventory.
...
CSS custom properties will hold the screenshot-derived palette and spacing. The page is one continuous deep blue-black stage with a restrained teal glow and CSS-only grid texture. Thin separators define the five regions; strong borders identify only selected states, the document ribbon, dialogs, and the toast. Regular content rows do not receive their own frames, elevated backgrounds, gradients, or shadows. Pale cream marks selected tabs, teal marks route and progress accents, and amber or red remains reserved for actions and warnings. Exact proprietary fonts are not required; the existing condensed system stack remains dependency-free.

Normal-content geometry is isolated in `src/layout.css`, which begins with the small set of values intended for manual adjustment: stage width, header and footer sizing, reward and control rail widths, region spacing, route-column widths, and document image height. One shared LiftKit-inspired proportional type scale in `src/typography.css` defines caption, label, body, heading, metric, and display sizes for both the wireframe and live interface; a single fluid root size scales those ratios between the authoritative 1440-pixel and 2560-pixel desktop widths, and components SHALL use the roles instead of one-off font-size values. A shared scale in `src/spacing.css` uses successive golden-ratio proportions for internal gaps, padding, and region spacing; components SHALL select a named step instead of declaring independent spacing formulas. `src/layout.css` owns grid, flex, position, dimensions, spacing, overflow, and the single responsive breakpoint. `src/styles.css` owns the palette, borders, backgrounds, emphasis, and interaction states and does not reposition normal content. This separation keeps the live renderer as the only markup source while making placement adjustable without tracing visual-state rules.
...
The 1440-to-2560-pixel desktop presentation is authoritative. One narrow-screen breakpoint retains every feature and stacks controls, results, rewards, and footer inventory in workflow order. The document tray may scroll internally; the viewport does not scroll horizontally.
...
A persistent footer or credits region will state: “Escape from Tarkov and all game image assets displayed here belong to Battlestate Games. This is an unofficial fan-made optimization tool.” It remains available without opening a modal and is separate from the dismissible cookie notice.
...
This boundary allows exhaustive unit testing and makes future data updates independent of rendering.

### 7. Resolve all remaining rewards and inventory transformations before route optimization

While any Battle Pass reward remains unclaimed, the engine always includes every unclaimed reward. It builds a legal redemption sequence from reward prerequisites and implicit page-unlock thresholds, and returns that sequence as the recommended order in which the player should take rewards. Requirements are accumulated by regular document type; there is no selected-reward or partial-pass goal.
...
Because Classified Documents are fungible backfill, the fixed maximum consumption quantity is allocated independently for Fastest and Safest so each profile eliminates its highest-cost remaining deficits. They may cover an entire reward when every required regular document is missing, but they are never chosen in place of matching regular documents already owned. Profile optimization decides where the fixed quantity is spent, not whether some legally usable owned Classified Documents are left unused.
...
The scheduler simulates Battle Pass progression instead of merely packing aggregate deficits by location. Until the next page unlocks, it prioritizes the lowest-objective-value claimable rewards on the current frontier page, farms only the missing documents for those rewards, redeems them as soon as their requirements are available, and records the newly unlocked page. After the final page unlocks, it clears every remaining reward. Each day remains within the fixed mode limit and groups work by location where possible.

Scheduling is secondary to the route recommendation. The initial view shows only the selected profile's next farming or claiming action, estimated redemption days, and routing factors. Regular-document deficits appear on the matching owned-document cards and zero deficits are hidden. It explicitly calls the result a planning estimate rather than a promise about calendar dates, raid count, or random spawns.

A `View full schedule` action opens a native dialog containing every estimated day as rule-separated manifest rows, its document targets, location grouping, routing factors, and rewards to claim after farming. The schedule does not use day cards, show `PAGE X UNLOCKED` labels, or assign real dates unless a later requirement establishes a trustworthy reset-time model. The left-column exclusive accordion shows one selected Battle Pass page. A selected page whose previous-page claim threshold is unmet shows `Claim N more from Page X`; that hint disappears immediately when the threshold is met.

The daily limit affects the schedule and estimated number of days, not the relative cost of two plans that require the same number of farmed documents.

### 11. Switch to a dedicated Black Division crate goal after all rewards are claimed

When every reward is marked claimed, the all-unclaimed-rewards plan switches to a crate-count control with a default of one. The engine first applies the player's regular-document inventory, identified by `kind: "regular"`, toward the 10:1 crate requirement, then assigns any shortage to the eligible farming location with the lowest `maxRaidTimeMin`. A larger user-selected crate count scales the requirement.
...
State will be split into versioned first-party cookies for progress, optimizer settings, and UI preferences to remain within per-cookie size limits. Persisted UI preferences include the selected reward page and route profile, but not the transient selected route stop or open state of either native dialog. Each envelope records the current `gameDataVersion` (`1.1.0.0.46657.8.6.2026` initially) and an independent integer cookie-schema version. The former identifies which game facts the selections were created against; the latter controls parsing and migration. Cookies will contain only compact JSON values, use a long but finite lifetime, and set `SameSite=Lax` and `Secure` when served over HTTPS. No game credentials or personal identifiers are stored.

On load, invalid or unknown cookie versions fall back safely to JSON defaults. Reset deletes every optimizer cookie and restores initial state. Cookie serialization and maximum-size behavior will be covered by tests.
...
Vitest will cover JSON validation and pure optimizer scenarios, including prerequisite closure, classified backfill, immediate TarCoin availability, bundle selection, difficulty mappings, maximum raid times, Fastest and Safest alternatives, low-hop tie-breaking, fixed mode limits, reward claims that unlock the next page, complete reward coverage by the schedule, and crate mode. Fake-timer tests will verify the exact season timestamp, localized countdown formatting, zero clamping, and timer shutdown. Localization validation tests will compare referenced IDs with `localization.json`, enforce unique entry IDs, verify every declared language value, and check item/image/screenshot description coverage. jsdom and Testing Library DOM will cover the five-region layout, dynamic page-threshold hints, compact left-column content, contiguous footer inventory entry and selected-profile deficits, tracking-only Claim all/Clear all, the persisted route toggle, one flat route manifest with visible factors, sibling plan disclosures, native full-schedule dialog, fixed mode limits without duplicate readouts, locale switching and persistence, cookie restoration, the dismissible persistence toast, persistent asset disclaimer, reset behavior, and the feedback preview.
