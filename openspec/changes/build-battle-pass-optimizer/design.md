...
Normal-content geometry is isolated in `src/layout.css`, which begins with the small set of values intended for manual adjustment: stage width, header and footer sizing, reward and control rail widths, region spacing, route-column widths, document image height, and the four-level type scale. It owns grid, flex, position, dimensions, spacing, overflow, and the single responsive breakpoint. `src/styles.css` owns the palette, borders, backgrounds, emphasis, and interaction states and does not reposition normal content. This separation keeps the live renderer as the only markup source while making placement adjustable without tracing visual-state rules.
