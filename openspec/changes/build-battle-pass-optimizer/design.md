...
The left column shows exactly one Battle Pass page at a time. Previous, next, and direct page controls navigate all pages; per-page and global Claim all/Clear all actions remain available. A visible reward row contains only the Battle Pass item name and a compact, shortened list of document requirements, plus its claimed control. It does not show reward artwork, long descriptions, stats, target selection, or unrelated metadata. Requirement abbreviations must remain unambiguous and expose their full localized text accessibly.
...
A `View full schedule` action opens a native dialog containing every estimated day as rule-separated manifest rows, its document targets, location grouping, routing factors, and rewards to claim after farming. The schedule does not use day cards, show `PAGE X UNLOCKED` labels, or assign real dates unless a later requirement establishes a trustworthy reset-time model. Left-column reward controls show one selected Battle Pass page. A selected page whose previous-page claim threshold is unmet shows `Claim N more from Page X`; that hint disappears immediately when the threshold is met.
