...
The reviewed Battle Pass workspace is the sole source HTML template. Local development, Playwright, and production builds use that template; no secondary `wireframe.html` preview is shipped. The build emits localized English-root and Russian `/ru/` HTML entries that import the same catalog, localization, persistence, state, redemption, and optimizer-worker modules rather than maintaining a second domain implementation.
...
One root `site.config.json` records the GitHub Pages base path, canonical root URL, repository URL, and locale routes. English uses the current root URL. Russian uses `/ru/`. The Vite build, sitemap generator, build validator, runtime locale navigation, and Playwright configuration read this file.

The source HTML contains meaningful default content and remains visible while catalogs load. Pending controls are inert and the shell reports a busy state. A successful initialization enables the shell. A failed initialization leaves controls disabled and shows a localized static error.

The build validates the five catalogs and emits `dist/index.html` and `dist/ru/index.html` from one template. Each page contains localized visible text, one `h1`, a self-referencing canonical, reciprocal `en` and `ru` language links, and an English-root `x-default`. Open Graph, X card, and WebApplication JSON-LD metadata use the page locale and canonical URL.

The locale URL is authoritative when it is explicit. The root checks the saved locale first and the browser locale second. A Russian result replaces the root URL with `/ru/` before initialization. The language selector saves the locale and navigates to its configured URL. Every locale route uses the same root-path cookies, so language navigation preserves optimizer state.

The footer opens a localized About dialog generated from validated catalogs. It explains page unlocks, daily limits, regular exchanges, Classified backfill, Fastest and Safest routing, catalog totals, and eligible map locations. It states that map names do not identify individual spawn points.

The generated sitemap contains only the two canonical application URLs. The project does not emit `robots.txt`, because the repository cannot control the hostname root. Optional Google Search Console verification uses a GitHub Actions repository variable and is omitted when it is empty.

An original 1200 by 630 social image uses only project-owned graphics. Each original document image produces 192 and 384 pixel WebP variants. The persistent ribbon loads these images lazily, while the current Focus image receives high fetch priority.
