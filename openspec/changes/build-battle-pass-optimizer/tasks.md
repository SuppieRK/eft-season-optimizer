...
- [x] 5.8 Implement minimum local real-money TarCoin-package estimation by normalizing active-locale numeric prices to currency fraction units, then excess TarCoins and package count; format results at runtime without a `FROM` label, reject incomplete or mixed-currency calculations, and keep it independent from farming routes.
- [x] 5.9 Add unit tests for global reward sequencing, prerequisites and page unlocks, claimed rewards, sufficient inventory, mixed and duplicate `5:1` regular exchanges, protected matching inventory, Classified exclusion from exchanges, partial and whole-reward Classified backfill, forced maximum Classified consumption before exchanges, zero remaining when possible, unavoidable surplus, no redeemable deficit, buyout with staged reward earnings, early-purchase top-up, bundle selection, circular-credit prevention, minimum local package cost, excess and package-count ties, and unavailable local prices.
...
- [x] 6.4 Hold maximum legal owned Classified consumption fixed while optimizing its deficit allocation and subsequent regular-document exchanges independently for the Fastest and Safest profile objectives.
...
- [x] 12.6 Apply the implicit previous-page reward-count-minus-one gate to every optimizer sequence, projection, and buyout path, with step-by-step regression coverage.
...
- [x] 16.49 Compute one semantic fingerprint across all public data catalogs, fetch catalogs without stale browser caching, migrate matching legacy cookie envelopes to the fingerprint format, and sanitize stale optimizer cookies against current catalogs on reload.
- [x] 16.50 Preserve all sanitized player state while migrating matching `gameDataVersion` cookie envelopes, and add unit plus Chromium regressions that verify inventory, settings, and UI preferences survive the first fingerprint-enabled reload.
- [x] 16.51 Replace per-document route cost with exhaustive useful-yield assignment and projected-raid cost, preserve Fastest/Safest factor ordering, expose every assigned location document as a priority, and add threshold, asymmetric-quantity, safety, insurance, and Chromium pressure regressions.
...
- [x] 16.57 Preserve valid document counts and claimed reward selections, along with compatible UI settings, across semantic catalog-fingerprint changes by sanitizing stale cookie payloads and rewriting them with the current fingerprint, with unit and Chromium regressions.
- [x] 16.58 Add 15 independent Chromium carryover scenarios that add, remove, and modify an entry in each of `documents.json`, `locations.json`, `battle-pass.json`, `optimizer-rules.json`, and `localization.json`; require successful initialization, a changed fingerprint, preserved document count and reward selection, rewritten cookies, and no console errors.
