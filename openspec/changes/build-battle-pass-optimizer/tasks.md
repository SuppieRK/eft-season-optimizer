...
- [x] 16.49 Compute one semantic fingerprint across all public data catalogs, fetch catalogs without stale browser caching, migrate matching legacy cookie envelopes to the fingerprint format, and replace genuinely stale optimizer cookies with current defaults on reload.
